...
;; Copyright (C) 1992, 1994, 2000, 2002 Free Software Foundation, Inc.

;; Author: Sebastian Kremer <sk@thp.uni-koeln.de>
;; Modified by: Francis J. Wright <F.J.Wright@qmul.ac.uk>
;; Maintainer: FSF
;; Keywords: unix, dired

;; $Id: ls-lisp.el,v 1.45 2002/10/12 16:17:32 fjw Exp $
...
;; will not work).  For example, copy ls-lisp.el to a file in your
;; load-path, byte-compile it, and include in your .emacs
;;
...
;; Modified by Francis J. Wright <F.J.Wright@qmul.ac.uk>, mainly to
...
0 and -1e+INF are (usually) both valid, meaning now or never."
  ;; Functionality suggested by Geert Ribbers <geert.ribbers@realworld.nl>
...
(or (featurep 'ls-lisp)  ; unless this file is being reloaded!
    (fset 'original-insert-directory (symbol-function 'insert-directory)))

;; Stub to allow ls-lisp to parse symbolic links via another library:
(or (featurep 'w32-symlinks)		; to allow reloading
...
platforms; see `ls-lisp-parse-symlink'."
...
for directory, string (name linked to) for symbolic link, or nil."
  (let ((file-name (cadr filedata)))	; file-type
    (if (or (stringp file-name)		; GNU/UNIX symlink?
	    (and (null file-name)	; non-GNU/UNIX symlink?
		 (stringp
		  (setq file-name (ls-lisp-parse-symlink (car filedata))))))
	(progn
    ...
    (concat (if (memq ?i switches)	; inode number
		...
		(let (end)
		  (when (and ls-lisp-hide-.lnk
			     (string= (substring file-name -4) ".lnk"))
...
(defun ls-lisp-human-size (file-size)
  ...
  ;; Functionality suggested by Sergej Malinovski <sergej@nospam.dk>
...
;;; Wildcard conversion and brace expansion
...
;;    "\\`[^ ]*\\.\\(c\\|h\\|cpp\\)\\'"
;; (ls-lisp-wildcard-to-regexp "*.{txt,texi}") ->
;;    "\\`[^ ]*\\.\\(txt\\|texi\\)\\'"
;; (ls-lisp-wildcard-to-regexp "*.{txt,}") -> "\\`[^ ]*\\.\\(txt\\|\\)\\'"
;; (ls-lisp-wildcard-to-regexp "*.{,txt}") -> "\\`[^ ]*\\.\\(\\|txt\\)\\'"

;; but these (correctly) are not:
...
;; Test examples from the bash manual:

;; (ls-lisp-wildcard-to-regexp "/usr/local/src/bash/{old,new,dist,bugs}") ->
;;    "\\`/usr/local/src/bash/\\(old\\|new\\|dist\\|bugs\\)\\'"

;; (ls-lisp-wildcard-to-regexp "/usr/{ucb/{ex,edit},lib/{ex?.?*,how_ex}}") ->
