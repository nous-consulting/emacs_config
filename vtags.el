...
(defvar *last-heuristic-tags-table* nil)
...
(defun find-tags-table-heuristically ()
  ...
  (let ((table (or
                ...
                (get-tagfileinfo (expand-file-name (format "%s/tags" (pigide-ensure-project))))
                ...
                *last-heuristic-tags-table*
                ...
                (get-tagfileinfo (expand-file-name
                                  (read-file-name "Visit tags table: (default TAGS) "
                                                  ...
                                                  "TAGS"
                                                  t))))))
    (when table (setq *last-heuristic-tags-table* table))))
...
(defun tags-completion-table-from-tagfile (string predicate tfi)
  ...
  (cond
       ((not (tagfileinfo-etags-p tfi))
        (vtags-find-and-return-table string predicate tfi))
       (t
       ;; Do etags tags
        (save-excursion
          (tags-get-buffer-and-verify-table tfi)
  ...
  (save-excursion
    ...
    (let ((table (make-vector 511 0)))
      (vtags-look pattern tfi
                  (lambda (line count)
                    (intern (tagEntryInfo-name (vtags-parse-line line)) table)))
      table)))
  ...
  (if (eq what t)
      (all-completions
           ...
           (tags-completion-table-from-tag-table-list
                string
                predicate
                (or tags-table-computed-list (list (find-tags-table-heuristically))))
           predicate)
    (try-completion
           string
           (tags-completion-table-from-tag-table-list
...
The current point and the destination of this search are added to the placeholder list."
  ...
  (let (
        ...
        (tables (or tags-table-computed-list (list (find-tags-table-heuristically))))
...
(defun tags-apropos (regexp)
  ...
  (with-output-to-temp-buffer "*Tags List*"
    ...
    (princ "':\n\n")
    (save-excursion
      (let ((tables (or tags-table-computed-list (list (find-tags-table-heuristically)))))
        (while tables
...
Returns the number of matched lines."
  ...
  (if (equal "0" (tagfileinfo-sorted tfi))
    ...
    (let ((vtags-look-buf (get-buffer-create "*Vtags-Look-Buffer*")) ; scratch buffer
          (blksize (* 4096 1)) ; big enough to hold all matching entries
      ...
      (insert (shell-command-to-string
               (format "look -b \"%s\" \"%s\" | grep -v \"from .* import\"" tag file)))
      (beginning-of-buffer)
      ;; END OF HACK
